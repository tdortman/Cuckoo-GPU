...
TEST_F(CuckooFilterTest, BasicDeletion) {
    ...
    for (size_t i = 0; i < keys.size(); ++i) {
        EXPECT_TRUE(output[i])
            << "Key " << keys[i] << " should be found before deletion";
    }
    ...
    for (size_t i = 0; i < keys.size(); ++i) {
        EXPECT_TRUE(deleteOutput[i])
            << "Key " << keys[i] << " should be successfully deleted";
    }

    EXPECT_EQ(remainingAfterDelete, 0)
        << "No keys should remain after deletion";
...
TEST_F(CuckooFilterTest, PartialDeletion) {
    ...
    thrust::device_vector<uint32_t> d_insertKeys(
        insertKeys.begin(), insertKeys.end()
    );
    ...
    thrust::device_vector<uint32_t> d_delete_keys(
        deleteKeys.begin(), deleteKeys.end()
    );
    thrust::device_vector<uint8_t> d_deleteOutput(deleteKeys.size());

    size_t remainingAfterDelete =
        filter.deleteMany(d_delete_keys, d_deleteOutput);
    ...
    for (size_t i = 0; i < deleteKeys.size(); ++i) {
        EXPECT_TRUE(deleteOutput[i])
            << "Key " << deleteKeys[i] << " should be deleted";
    }

    EXPECT_EQ(remainingAfterDelete, 5)
        << "5 keys should remain after deleting 5";
    ...
    thrust::device_vector<uint32_t> d_remainingKeys(
        remainingKeys.begin(), remainingKeys.end()
    );
    ...
    thrust::copy(
        d_remainingOutput.begin(),
        d_remainingOutput.end(),
        remainingOutput.begin()
    );

    for (size_t i = 0; i < remainingKeys.size(); ++i) {
        EXPECT_TRUE(remainingOutput[i])
            << "Key " << remainingKeys[i] << " should still be found";
    }
}
...
TEST_F(CuckooFilterTest, DeleteAndReinsert) {
    ...
    EXPECT_EQ(reinserted, keys.size())
        << "Should be able to reinsert deleted keys";
    ...
    for (size_t i = 0; i < keys.size(); ++i) {
        EXPECT_TRUE(output[i])
            << "Key " << keys[i] << " should be found after reinsertion";
    }
}
...
TEST_F(CuckooFilterTest, LoadFactorAfterDeletion) {
    ...
    EXPECT_GT(loadFactorBefore, 0.0f)
        << "Load factor should be positive after insertion";

    size_t remaining = filter.deleteMany(d_keys);
    float loadFactorAfter = filter.loadFactor();

    EXPECT_LT(loadFactorAfter, loadFactorBefore)
        << "Load factor should decrease after deletion";
    EXPECT_FLOAT_EQ(
        loadFactorAfter, static_cast<float>(remaining) / filter.capacity()
    );
}
...
TEST_F(CuckooFilterTest, DeleteDuplicates) {
    ...
    size_t successfulDeletions =
        std::count(deleteOutput.begin(), deleteOutput.end(), true);
